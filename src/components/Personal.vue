...
<div id="app1" class="hero">
  <h3 class="vue-title"><i class="fa fa-smile" style="padding: 3px"></i>{{messagetitle}}</h3>
  <h4 class="vue-title2">Welcome {{$store.state.username}}</h4>
...
export default {
  ...
  data () {
    return {
      ...
      messagetitle: 'Personal Page',
      stories: [],
      errors: [],
      props: ['_id'],
      columns: ['title', 'type', 'class', 'content', 'written_times', 'upvotes', 'upvote', 'downvote', 'edit', 'remove'],
      options: {
        perPage: 10,
  ...
  created () {
    this.loadStories(this.$store.state.username)
  },
  methods: {
    loadStories: function (username) {
      storyservice.fetchMyStories(username)
        .then(response => {
